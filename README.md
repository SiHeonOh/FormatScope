...
|---|---|---|---|---|---|---|---|
| fp32 | 32 | 86.34 | — | — | — | — | — |
| int4 | 4 | 61.65 | 81.36 | 27,906 | 27,772 | 8,907 | 9,722 |
| int8 | 8 | 86.39 | 86.03 | 100,177 | 98,811 | 10,719 | 12,197 |
| fp8e4m3 | 8 | 85.50 | 85.78 | 111,259 | 111,272 | 35,040 | 36,100 |
| mxint8 | 8.25 | 86.39 | 86.19 | 110,692 | 109,630 | 31,478 | 35,952 |
| mxfp4 | 4.25 | 60.71 | 82.79 | 35,967 | 35,294 | 26,451 | 31,201 |
| int4_b32 | 4.25 | 78.46 | 83.21 | 36,815 | 35,876 | 26,907 | 30,842 |

"Bits per number" amortizes the 8-bit shared scale of the block-32 formats over the block. `int4_b32` (INT4 elements with a block-32 power-of-two scale) separates how much of MXFP4's behaviour comes from block scaling and how much from the E2M1 encoding. It has a verified unit too (`rtl/int4_b32/`, the MXINT8 design at 4 bits, decision A1), synthesized and timed in the same flow as the other five.
...
**H1 — a fused FP8 dot product costs far less than a single FP8 MAC suggests.** At the shared clock, the FP8-E4M3 DP32 is **12.6% larger than INT8** (111,272 vs 98,811 µm²). The single-MAC estimate in van Baalen et al. is 50–180%. **That is an area comparison at a relaxed clock, not at equal speed:** T1 is set by the slowest unit, and at it the FP8 path takes 36.1 ns against INT8's 12.2 ns, so the FP8 unit is about 3× slower as well as 12.6% larger (about 3.3× in area × delay). Both units are single-cycle (D14); matching INT8's throughput would mean pipelining the fused stage, and those registers are area this number does not include, so 12.6% is a lower bound. The difference from the single-MAC estimate is the fused stage: the 32 products and the accumulator are aligned once, summed in one integer tree, and normalized and rounded once, so the per-lane cost is a 4×4-bit significand multiplier plus a shifter rather than a full floating-point adder. Where the remaining cost sits, from the hierarchical synthesis (`results/breakdown.csv`, `results/figures/breakdown.png`): in INT8 the 32 multipliers are 77% of the unit (76,637 µm²) and the tree 16%. In FP8 the multipliers shrink to 17,991 µm² and the **alignment stage becomes 54% of the unit (59,739 µm²)**, with the wider tree at 28,900 and the decoders, normalize, and accumulate together under 10k. The FP8 penalty is alignment, not multiplication. MXINT8 keeps INT8's multipliers and tree within 10% and pays its increment in the two-term align (6,819) and normalize (3,526) stages; MXFP4 does the same over INT4.

**H2 — block scaling is the cheap lever; it holds, and the accuracy half needs the fine-tune.** On the hardware side MXINT8 costs **+11.0%** over INT8 and MXFP4 **+27.1%** over INT4 for 0.25 more bits per number. On the accuracy side, after PTQ alone the hypothesis would fail: MXFP4 (60.71%) sits *below* plain INT4 (61.65%), 25.68 points under INT8. After the five-epoch fine-tune MXFP4 reaches **82.79%**, 3.24 points under INT8 given the same fine-tune (86.03%), so it closes 87% of the gap at about a third of INT8's area (35,294 vs 98,811 µm²). MXINT8 matches INT8 before and after (86.39 / 86.19 against 86.39 / 86.03): on this model its 11% buys no accuracy, because per-channel scales already do the job at 8 bits. The `int4_b32` ablation says which part of MXFP4 matters: INT4 elements with the same block-32 scale reach 78.46% after PTQ and **83.21%** after the fine-tune, level with or ahead of MXFP4 at both stages, so the block scale carries the gain and the E2M1 element encoding adds nothing measurable here. Its unit costs 35,876 µm² at T1, 1.7% more than MXFP4's 35,294 for 0.42 more points, so the two sit within noise of each other on both axes and E2M1 buys neither accuracy nor area on this model. These are single-seed runs and 0.4 points is inside their noise; Chen et al. find the opposite ordering at 4 bits on LLMs (see Related work). The full write-up is in [`docs/results.md`](docs/results.md).

**Alignment window.** Widening the fused stage's window from 24 to 32 bits costs FP8 +17% area (129,825 vs 111,272 µm² at T1) for no delay gain; MXINT8, MXFP4, and INT4-b32 move under 4%. 24 is the default.
